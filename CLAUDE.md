...
## 개발 환경 vs 운영 환경 (Mono Docker)

| | 개발 환경 (`lila.conf`) | 운영 환경 (`mono.conf`) |
|---|---|---|
| **구성** | Docker Compose 멀티 컨테이너 | 단일 mono 이미지 |
| **설정 파일** | `conf/lila.conf` | `conf/mono.conf` |
| **Caddy 설정** | `conf/Caddyfile` | `conf/mono.Caddyfile` |
| **서비스 연결** | 컨테이너 이름 (예: `mongodb`, `redis`) | 환경변수 (예: `${?LILA_DOMAIN}`) |
| **부가 서비스** | gif, search, picfit, push 등 개별 설정 | 최소 설정만 포함 |
| **외부 API** | 엔드포인트 직접 지정 | 환경변수 또는 누락 가능 |

### 기능 추가 시 운영 환경 체크리스트

새 기능이 외부 서비스나 설정에 의존하는 경우, **반드시 `mono.conf`에도 해당 설정이 있는지 확인**:

1. `lila.conf`에 새 설정을 추가했다면 → `mono.conf`에도 대응 설정 추가 (환경변수 바인딩)
2. `base.conf`의 기본값이 `localhost:*`인 항목은 mono 환경에서 연결 실패 → 외부 엔드포인트 명시 필요
3. Caddy 프록시 경로를 추가했다면 → `mono.Caddyfile`에도 반영
...
## CI/CD (Publish Image 워크플로우)

### 빌드 구조

`docker/mono.Dockerfile`은 사전 빌드된 아티팩트를 COPY하는 방식. sbt/node 빌드는 Docker 외부에서 수행.

```
Phase 1 (병렬):
  build-node     (ubuntu-latest)       Node assets 빌드 (pnpm 캐시)
  build-sbt      (matrix: amd64/arm64) lila + lila-ws + lila-fishnet (sbt 캐시, 네이티브 러너)

Phase 2 (Phase 1 완료 후, 병렬):
  docker-image   (matrix: amd64/arm64) dbbuilder + 최종 이미지 조립 (Docker 레이어 캐시)

Phase 3:
  manifest       (ubuntu-latest)       멀티플랫폼 매니페스트 병합
```
...
| 대상 | 메커니즘 | 비고 |
|------|---------|------|
| sbt coursier/ivy2 | `actions/setup-java` `cache: sbt` | JitPack 장애 시에도 캐시 히트로 빌드 가능 |
| pnpm store | `actions/setup-node` `cache: pnpm` | |
| Docker 레이어 | `cache-from/to: type=gha` | dbbuilder 스테이지 캐싱 |

### 트리거

- `push tags: v*` — 릴리스 태그
- `workflow_dispatch` — 수동 (repo, branch, platforms 선택 가능)

### 핵심 파일

- `.github/workflows/publish-image.yml` — 워크플로우 정의
- `docker/mono.Dockerfile` — 아티팩트 기반 Docker 이미지 (빌더 스테이지 없음, dbbuilder만 유지)
- `conf/supervisord.conf` — 최종 이미지 서비스 관리 (lila, lila-ws, lila-fishnet, mongo, redis, caddy, fishnet)

### 주의사항

- `mono.Dockerfile`은 직접 `sbt`/`node` 빌드를 하지 않음 — `artifacts/` 디렉토리에서 사전 빌드된 결과물을 COPY
- `artifacts/` 경로 구조는 `supervisord.conf`의 바이너리 경로와 일치해야 함
- JitPack 의존성(`scalachess`, `scalalib`, `compression`)은 첫 성공 빌드 후 sbt 캐시에 저장되어 이후 장애 영향 없음
